...
def pull_latest_photo(photo_count):
    try:
        os.makedirs('images', exist_ok=True)
        photo_name = f"images/{photo_count}.jpg"
        ...
        run_adb_command(f'pull {latest_photo} {photo_name}')
...
def capture_photos():
    try:
        ...
        while capture_flag:
            ...
            tap_screen(550, 1990)
            time.sleep(16.4)  # Chờ đợi thời gian chụp ảnh hoàn thành
            photo_count += 1
            image_path = pull_latest_photo(photo_count)
            if image_path:
                display_image(image_path)
                image_paths.append(image_path)
                update_photo_count()  # Cập nhật số lượng ảnh đã chụp
            # Tính toán thời gian chờ để giữ cho chu kỳ chụp ảnh là 16.4 giây
            elapsed_time = time.time() - start_time
            time_to_wait = max(0, 16.4 - elapsed_time)
...
def create_video_from_images():
    try:
        ...
        video = cv2.VideoWriter(video_name, cv2.VideoWriter_fourcc(*'mp4v'), 25, (width, height))  # Sử dụng định dạng mp4
        
        sorted_image_paths = sorted(image_paths, key=lambda x: int(os.path.basename(x).split('.')[0]))  # Sắp xếp các ảnh theo thứ tự tên file
